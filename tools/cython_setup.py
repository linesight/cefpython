...
def set_compiler_options(options):
    ...
    if WINDOWS:
        ...
        extra_compile_args.extend(["/EHsc", "/wd4305", "/std:c++20", "/DNOMINMAX"])
...
def compile_time_constants():
    ...
    # platform_cimports.pxi: cimports for the main cefpython.pyx (replaces IF blocks)
    with open(os.path.join(SRC_DIR, "platform_cimports.pxi"), "wb") as fo:
        contents = "# This file was generated by setup.py\n"
        contents += "\n# Type this command to ignore changes to this file:\n"
        contents += "#   git update-index --assume-unchanged src/platform_cimports.pxi\n\n"
        if WINDOWS:
            contents += "from windows cimport *\n"
            contents += "from dpi_aware_win cimport *\n"
        elif MAC:
            contents += "from mac cimport *\n"
        elif LINUX:
            contents += "from linux cimport *\n"
            contents += "cimport x11\n"
        fo.write(contents.encode("utf-8"))

    # extern/cef/platform_cimports.pxi: cimport for pxd files in extern/cef/
    cef_extern_dir = os.path.join(SRC_DIR, "extern", "cef")
    with open(os.path.join(cef_extern_dir, "platform_cimports.pxi"), "wb") as fo:
        contents = "# This file was generated by setup.py\n"
        contents += "\n# Type this command to ignore changes to this file:\n"
        contents += "#   git update-index --assume-unchanged src/extern/cef/platform_cimports.pxi\n\n"
        if WINDOWS:
            contents += "from cef_win cimport *\n"
        elif MAC:
            contents += "from cef_mac cimport *\n"
        elif LINUX:
            contents += "from cef_linux cimport *\n"
        fo.write(contents.encode("utf-8"))
